...
export type Invitation = {
   ...
   *
   * **【2026-09-18 訂正(`V19-M2-T00`)。上の行を1バイトも消していない】**
   * **「8列目は持てない」は今日も真である。** **一方で「区別できない」は今日は偽である**
   * —— **取り消しはこの列に**素の時刻ではない値**を書き、状態は
   * `src/auth/invitations.ts` の1関数が3値(未使用 / 使用済み / 取り消し済み)で返す**
   * (`ADR-0452`)。 **応答には状態が**別のキー**で載り、この項目は今日どおり
   * ISO8601 のまま返る**(印を応答に出さない)。
