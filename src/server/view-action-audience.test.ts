      ...
      roles: [
        ...
        // **(A-1) の 403 と (C-1) の 403 はそこで立っているからである。**
        {
          ...
          rules: [
            ...
            //
            // **直上の「画面(`view`)とボタン(`action`)の規則は `customer` に1本も
            // 書いていない」は、画面については今日は偽である(1バイトも消していない)。**
            // **足すのは `catalog-list`(規則を1本も持たない客用の一覧)だけであり、
            // (A-1) が測っている `admin-product-list` には1本も足していない** ——
            // **(A-1) の 403 は今日もそのまま立つ。** **ボタンの規則も1本も足していない**
            // (**(C-1) の 403 はそこで立っている**)。
            // **(B-1) が測っているのは項目の規則が値を落とすことなので、主張(`expect`)は
            // 1バイトも書き換えていない。**
