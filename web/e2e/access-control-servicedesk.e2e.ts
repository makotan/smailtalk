...
test("V7-M7-T06 (iv) アサインされた人は課題を GET でき、コメントの POST が通る", async ({
  ...
  try {
    ...
    // ないためである。**
    //
    // **【`V15-M6B-T01`(`CR-G1` / `ADR-0404`)による打ち直し。上の【限界】の一段落を
    // 1バイトも消していない。テスト名も1文字も変えていない】**
    //
    // **旧の期待値(逐語)**: `expect(strangerComment.status, tr.text()).toBe(201);`
    // **今日は 403 である。**
    //
    // **なぜ変わったか** —— **`comments` は `inherit_from: ["issue"]` を宣言した表であり**
    // (上の `declaration("comment", "writer", ["issue"])`)、**`V15-M2` が「その表に行を
    // 作るには、指した親の行に書けなければならない」という前提の関門を単件の作成に足した。**
    // **デイブは親の課題への付与を1件も持たない** —— **直前の `outsider` の `GET` が 404 で
    // あることが、まさにそれを示している。** **したがって作成も断られる。**
    //
    // **【この打ち直しは検査を緩めていない。逆である】** —— **旧は「穴が開いている」ことを
    // 固定しており、今日は「その穴が塞がっている」ことを固定する。** **測っている構成は
    // 1バイトも変えていない**(同じアプリ・同じ3人・同じ要求の並び)。
    // **直後の 404 はそのまま残す** —— **「作れない」ことと「読めない」ことが同じ1本の
    // テストの中で並ぶので、片方だけを見て読み違えられない。**
    //
    // **【なぜ `V15-M2` 〜 `V15-M5` のどの段でも赤くならなかったか。丸めない】** ——
    // **`bun test`(単体)は `.e2e.ts` を1本も拾わない**(既知の躓き)。
    // **`V15-M2` は双子の単体検査 `src/server/role-grant-union.test.ts` の `(J)` を
    // 201 → 403 に打ち直したが、この E2E には気づいていない** ——
    // **`bun run test:e2e` を別に走らせるまで、機械は毎回通していたのに赤が誰にも見えなかった**
    // (`docs/plan/v15/records/v15-m6.md` §8-2 / §8-3 の実測)。
    //
    // **【要求の見出しは逐語で残してある】** —— **下の `"【限界】…"` という見出しは、
    // 実測の記録(transcript)にそのまま出る文字列である。** **1バイトも書き換えていない**
    // ——**この一段落が、その見出しが今日は限界を指していないことを述べている。**
    ...
    expect(strangerComment.status, tr.text()).toBe(403);
    // **ただし、その課題そのものは今も読めない**(コメントを作れても親は開かない)。
    // **【`V15-M6B-T01` による訂正。上の1行と下の見出しを1バイトも消していない】** ——
    // **括弧の中は今日は偽である**(デイブはコメントを1件も作れていない)。
    // **この 404 を残すのは、「作れない」ことと「読めない」ことが同じ検査の中に並ぶからである**
    // —— **どちらか片方だけが成り立つ状態を、この1本が今日も許さない。**
