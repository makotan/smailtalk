...
function AuthenticatedWorkspace({
   ...
   * **その API はもう無認証ではない**(`manifest` は `V8-M21` から、`changelog` と
   * 要件定義書は本段から、それぞれログインが要る)。 **3本が同列であることは今日も真である。**
   * **owner 限定にしない判断そのものは覆っていない** —— **掛かった関門はセッションの有無だけを
   * 見て、役割を1つも見ないからである**(閲覧者も顧客もログインしていれば読める)。
   */
   ...
   *
   * **【`V17-M4-T02` による訂正。上の4行は1バイトも消していない】** **いつ偽になったかを分けて書く**:
   *  - **`GET /manifest` が「認証を要求しない」は `V8-M21`(2026-08-10)から偽である** ——
   *    **本段が偽にしたものではない。**
   *  - **`GET /api/apps/:app_id/requirements` が「認証を要求しない」は本段(台帳 `AC-G20`)から
   *    偽である。**
   * **【それでも最後の1行は今日も真である。丸めない】** —— **掛かったのはセッションの有無だけを
   * 見る関門であり、役割を1つも見ない。** **ログイン済みの `customer` が API を直接叩けば
   * 今日どおり読める。** **UI が見せるものは API が許すものより狭いままである。**
   *
