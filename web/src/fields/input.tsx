 ...
 *
 * **【2026-09-16 訂正(`V18-M8-T02b` / `D-V18-33` / `ADR-0445` §Decision 4・§Decision 5・§Decision 7)。
 * 上の doc を1バイトも消していない】**
 * **上の逐語のうち2つが、今日の正ではない。**
 *
 * 1. **「`inherit_from` も `grant.member` も1度も参照しない —— サーバの `rowGrantWriteJudge`
 *    がそう判定しているからである。」** …… **前半(この関数が `grant.table` だけを見ること)は
 *    今日も真である。** **偽になったのは**理由**のほうである** —— **今日の `rowGrantWriteJudge` は、
 *    付与表の相手の欄(`grant.member` / `grant.group`)に掛かる項目規則を当て、付与表そのものが
 *    `inherit_from` を宣言して対象の欄を指す形では、その親の関門も当てる。**
 * 2. **「`grant_write` が答えるのは「この行に付与を配れるか」で、親は1段も要らない。」** ……
 *    **「親は1段も要らない」は今日は偽である** —— **付与表そのものの `inherit_from` が対象の欄を
 *    指していれば、その親の関門が**表示の時点で**当たる。** **一方、対象行の権限を引き継ぎで辿ることは
 *    今日1段もしない**(`AC-G30b` の (ii) の実装は覆した。`ADR-0445` §Decision 7)。
 *
 * **この関数の本体は1バイトも変えていない** —— **変わったのはサーバ側の判定であり、ここが書いていた
 * 「サーバがそう判定しているから」という理由づけが、今日の正でなくなっただけである。**
 * **【禁止】これを「表示と実際が一致した」と読まない** —— **写していない断りは今日も在る**
 * ((3)(4)(5) と持ち主の偽装・条件つきの規則。`ADR-0445` §Decision 4 の表)。
 */
