...
describe("(5) 新しい読取経路を作っていない", () => {
   ...
   *
   * **【`V18-M7-T05` / `PM-G5` / `ADR-0444` §Decision 5 の 行6 による更新。上の3つの記述を
   *   1バイトも消していない】** **宣言どおり、この検査は実際に赤くなった。** **足したのは
   * `readCascadeDeleteDenial` の1本だけである**(43 → **44**)—— **サーバが `V18-M7-T02` で足した
   * 断り2本(③ = ぶら下がっている行のうち消せないものが在る `403` / ④ ⑤ = 件数の印が無い・合わない
   * `409`)を、詳細画面が既存の `isForbidden` / `isWriteConflict` と**区別する**ために要る。**
   * **【本ファイルの主題を1ミリも緩めていない】** **足した1本は `fetch` を1度も呼ばず、
   * レコードを1行も読まない** —— **すでに手元に在る `ApiError` を読むだけの純関数である。**
   * **新しい HTTP の口を1本も足していない**(`HTTP_ENTRY_POINTS` は1つも動かしていない)——
   * **撃つ先は着手前と同じ `DELETE .../records/:record_id` 1本で、載せる印が1本増えただけである。**
   * **【緩めていない】** **突き合わせは今日も全量の集合一致であり、2本目を足せば赤くなる。**
   */
  const API_EXPORTS = [
    // **【`V19-M3-T02` が足した1本】** **運営の画面から招待を1件取り消す口を呼ぶ(`SV-G4`)。**
    // **【新しい HTTP の口を1本も足していない】** —— **撃つ先は `issueInvitation` と
    //   **同じ** `POST /api/apps/:app_id/auth/invitations` 1本ちょうどで、送る中身が
    //   `{ username, revoke: true }` になるだけである。** **`DELETE` のルートを1本も足して
    //   いない**(`ADR-0336` 限定5 / 限定18)。 **`HTTP_ENTRY_POINTS` は **53** のまま
    //   1つも動いていない**(`src/server/entry-point-inventory.test.ts`)。
    // **【この1行を足す裁定もメインが行った。`T00` に続いて2度目である】** ——
    //   **`§9-E` の止まる条件4 に担い手が実際に当たって手を止め、メインが許した。**
    //   **メインは差し戻す前に7行すべてを自分で打ち直し、赤が本ファイルの1本だけであること、
    //   担い手が既存の検査を1バイトも書き換えていないこと、`apps/smailtalk/src` に
    //   0バイトであることを確かめている。**
    // **【本ファイルの主題に当たらない理由】** **本 describe の名は「(5) 新しい**読取**経路を
    //   作っていない」である。** **`revokeInvitation` は書込であり、レコードを1行も読まない。**
    //   **参照候補の集合にも並びにも1ミリも関わらない。**
    // **【誇張しない。ここに穴が在る】** **本ファイルのこの集合は `toEqual` の全量一致なので、
    //   画面の段が `api.ts` に関数を足すたびに必ず赤くなる。** **`T00` と `T02` で2度続けて
    //   同じ理由で赤くなった。** **「メインが許した」を繰り返すと、この検査が守っている
    //   「読取経路を増やしていない」という担保そのものが形骸化する。**
    //   **`V19-M3` はこれを穴として `docs/plan/v19/unfixed-holes.md` に立て、形を変えるべきかの
    //   判断を後続へ送る。** **本段では形を変えていない。**
    "revokeInvitation",
    // **【`V19-M3-T00` が足した1本】** **運営の画面から相手を1人招く口を呼ぶ(`SV-G1`)。**
    // **【新しい HTTP の口を1本も足していない】** —— **撃つ先は着手前から在る
    //   `POST /api/apps/:app_id/auth/invitations` 1本ちょうどで、`HTTP_ENTRY_POINTS` は
    //   **53** のまま1つも動いていない**(`src/server/entry-point-inventory.test.ts:354`)。
    // **【この1行を足す裁定はメインが行った】** —— **本段の計画 `docs/plan/v19/05-v19-m3-plan.md`
    //   `§9-E` の止まる条件4(既存検査の期待値を書き換える必要が出たら止める)に担い手が
    //   実際に当たって手を止め、メインが許した。** **どの ADR もこの集合を凍結していない**
    //   (実測: `docs/adr/` の全文で `API_EXPORTS` の当たりは **0件**)。
    // **【緩めていない】** **上の doc が書くとおり、突き合わせは今日も全量の集合一致である。**
    "issueInvitation",
    // **【`V18-M7-T05` が足した1本】** **ぶら下がっている行の断り(403 / 409)を見分ける純関数。**
